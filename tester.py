...
from parsers.context_parser import ContextParser  # tetap dipakai untuk future hooks

logger = logging.getLogger("xsscanner.tester")

# --- Simple console/Panel stubs to avoid rich dependency ---
...
def _spinner_loop():
    global _SPINNER_THREAD, _SPINNER_STOP, _PROGRESS_ACTIVE
    i = 0
...
def _print_progress(msg: str) -> None:
    # Update base message and ensure spinner is animating
    global _PROGRESS_MSG
    _PROGRESS_MSG = (msg or "").strip()
...
def _progress_newline() -> None:
    ...
    _SPINNER_STOP = None
    if _PROGRESS_ACTIVE:
        # Finish the current progress line cleanly
        print()
        _PROGRESS_ACTIVE = False
...
class XSSTester:
    ...
    def process_revisits(self, max_items: int = 5) -> None:
        ...
        for task in ready:
            ...
            _progress_newline()
            print(color(f"[Revisit] {url} (marker={marker})", "cyan"))
            try:
    ...
    # ===== Orkestrasi Phases =====
    def test_parameter(self, url: str, method: str, name: str, template: Dict[str, str], is_form: bool):
        method = method.upper()
        print(f"[Start] Testing param='{name}' method={method} url={url}")
        ...
        sanitizer_map = analyze_param_sanitizer(url, name, template, method, is_form)
        ...
        _progress_newline()
        print(color("[Phase 2] Encoding variants…", "cyan"))
        ...
        _progress_newline()
        print(color("[Phase 3] Quick probes: OOB, Header, Path/Fragment, Stored…", "cyan"))
        ...
        _progress_newline()
        print(color("[Phase 4] CSP-aware & nonce/hash bypass…", "cyan"))
        ...
        _progress_newline()
        print(color("[Phase 5] Probe → Progressive (engine)…", "cyan"))
        ...
        _progress_newline()
        print(color("[Phase 6] Coverage-guided (CDP)…", "cyan"))
        ...
        _progress_newline()
        print(color("[Phase 7] Static context-aware brute (parallel)…", "cyan"))
        ...
        for raw in DEFAULT_XSS_PAYLOADS["html_tag_injection"]:
            for tech, variant in generate_encoding_variants(raw).items():
                ...
                if confirm_execution(target_url):
                    _progress_newline()
                    print("=== Encoding Execution Success ===")
