...
class WAFDetector:
                ...
                if v:
                    ...
                    if meta:
                        ...
                        if meta.get('trust_proxy') is not None:
                            prof.trust_proxy = bool(meta['trust_proxy'])
                    break
        # Sync detector-level toggles based on profile insights
    ...
    def _match(self, resp) -> tuple[str, str, str, bool, Dict[str, Any], Dict[str, Any]]:
        ...
        if resp.status_code in (403, 406, 501):
            if re.search(r"access\s*denied|request\s*blocked|web\s*application\s*firewall|the\s*request\s*could\s*not\s*be\s*satisfied|generated\s*by\s*cloudfront", (resp.text or ''), re.I):
                matches = {"headers": [], "cookies": [], "body": [], "status": [resp.status_code]}
                return 'generic', 'low', 'none', False, matches, {}
        ...
        try:
            ...
            for token, vendor in heur:
                if token in hdr_dump:
                    matches = {"headers": [], "cookies": [], "body": [], "status": [resp.status_code] if resp.status_code else []}
                    return vendor, 'low', 'none', (resp.status_code == 429), matches, {}
        except Exception:
            pass
        ...
        try:
            if self.trust_proxy and hasattr(resp, 'history'):
                for h in (resp.history or []):
                    ...
                    l = (loc or '').lower()
                    if any(s in l for s in ('/cdn-cgi/', 'challenge', 'captcha', '_incapsula_', 'blocked', 'accessdenied')):
                        ev_type = 'challenged_js'
                        break
        except Exception:
            pass

        if rl or status == 429 or re.search(r"rate\s*limit|too\s*many\s*requests", body, re.I):
            ev_type = 'rate_limited'
        elif status in (403, 406, 501):
            ev_type = 'blocked'
        elif ch != 'none' or status in (503,):
            ev_type = 'challenged_js'
        # captcha markers
        try:
            if re.search(r"(g-recaptcha|h-captcha|recaptcha|data-sitekey)", body, re.I):
