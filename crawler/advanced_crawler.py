...
class AdvancedXSSCrawler(BaseCrawler):
    ...
    def crawl_and_discover_parameters(self):
        """
        Loop utama crawling:
          • Context persistent via storage_state_file (jika ada)
          • Auto-login (opsional) sebelum mulai crawl
          • BFS sederhana pakai single tab (stabil & hemat resource)
        """
        with sync_playwright() as pw:
            browser = pw.chromium.launch(headless=True)

            # Pakai persistent storage state kalau file ada
            ctx_kwargs = {}
            ...
            # Hook SPA router to discover routes from pushState/replaceState/hashchange
            self._install_spa_hook(context)

            # Hemat resource: abort sebagian besar asset berat (image/media/font/css)
            ...
            # Sync cookie dari requests.Session ke context (best-effort)
            try:
            ...
            # Create small page pool to simulate parallelism while preserving context/session
            ...
            while not self.to_visit.empty():
                ...
                try:
                    # Round-robin assign a page
                    pg = pages[(len(self.visited) + self.to_visit.qsize()) % len(pages)]
                    self._process_page(pg, url, depth)
                    # Stimulate DOM-based sources on each page to explore DOM-XSS pathways
                    try:
